...
async def run_test():
    ...
    try:
        ...
        # -> Type raf@raf.com into the email field (index 6), then type admin123 into the password field (index 7), then click Sign In (index 10).
        ...
        await expect(frame.locator('text=Products').first).to_be_visible(timeout=3000)
        await expect(frame.locator('text=Add Product').first).to_be_visible(timeout=3000)
        await expect(frame.locator('text=UI E2E Product 001').first).to_be_visible(timeout=3000)
