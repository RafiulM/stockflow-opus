...
async def run_test():
    ...
    try:
        ...
        # -> Fill the email and password fields (indexes 3 and 4) with provided credentials and click the 'Sign In' button (index 7). After the click, wait for the next page state and then verify the URL contains '/'.
        ...
        frame = context.pages[-1]
        assert '/' in frame.url
        await expect(frame.locator('text=Enter SKU').first).to_be_visible(timeout=3000)
