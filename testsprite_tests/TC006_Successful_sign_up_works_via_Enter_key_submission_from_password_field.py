...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'Sign up' link to open the signup form page
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/main/div/div/div[2]/form/p/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Fill the Full Name field (index 107) with 'Enter Submit User' (then fill email and password, and press Enter).
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/main/div/div/div[2]/form/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('Enter Submit User')
        
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/main/div/div/div[2]/form/div[2]/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('new.user+e2e.enter.001@example.com')
        ...
        # -> Fill Confirm Password (index 119) with 'ValidPassw0rd!' and press Enter to submit the form, then verify navigation/visibility of dashboard.
        frame = context.pages[-1]
        # Input text
