...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'Audit' (Audit Trail) link in the left sidebar to open the Audit page so the SKU search can be performed.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[2]/aside/div/nav/a[5]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Type 'SKU-TEST-001' into the SKU search input and click the 'Trace' button to run the first search.
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div[2]/div/main/div/div[2]/div/form/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('SKU-TEST-001')
        ...
        # -> Type 'SKU-TEST-002' into the SKU search input and click the 'Trace' button to run the second search (then verify the product card and timeline update to SKU-TEST-002).
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div[2]/div/main/div/div[2]/div/form/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('SKU-TEST-002')
        ...
        # Verify URL contains '/'
        assert "/" in frame.url
        # Verify the SKU input is visible and contains the updated SKU 'SKU-TEST-002'
        elem = frame.locator('xpath=/html/body/div[2]/div/main/div/div[2]/div/form/input').nth(0)
        await elem.wait_for(state='visible', timeout=5000)
        val = await elem.input_value()
        assert val == 'SKU-TEST-002', f"Expected SKU input value to be 'SKU-TEST-002', got '{val}'"
