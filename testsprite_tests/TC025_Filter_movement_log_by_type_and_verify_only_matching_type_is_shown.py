...
async def run_test():
    ...
    try:
        ...
        # -> Input the provided email into the email field (index 4).
        ...
        # -> Click the movement type filter combobox (index 826) to open the dropdown so the 'Inbound' option can be selected.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[2]/div/main/div/div[2]/div/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Click the 'Inbound' option in the movement type listbox to apply the filter, then wait for the movement log to update.
        frame = context.pages[-1]
        # Click element
        ...
        assert "/" in frame.url
        # Verify URL contains /movements
        assert "/movements" in frame.url
        # Verify the 'Inbound' filter option is visible in the movement log UI
        elem = frame.locator('xpath=/html/body/div[2]/div/main/div/div[2]/div/div/button').nth(0)
