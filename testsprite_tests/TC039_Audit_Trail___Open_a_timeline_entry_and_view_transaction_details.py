...
async def run_test():
    ...
    try:
        ...
        # -> Type the provided username into the email field, type the provided password into the password field, then click the Sign In button
        ...
        # -> Click 'Audit Trail' in the sidebar navigation to open the Audit page (use the sidebar 'Audit Trail' link).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[2]/aside/div/nav/a[5]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Type 'SKU-TEST-001' into the SKU search input and click the Trace button to load results.
        frame = context.pages[-1]
        # Input text
        ...
        frame = context.pages[-1]
        assert '/' in frame.url
        await expect(frame.locator('xpath=//h2[normalize-space()="Timeline"]').first).to_be_visible(timeout=3000)
        ...
        await expect(frame.locator('text=Quantity').first).to_be_visible(timeout=3000)
